...
#include <TVout.h>
#include <fontALL.h>
#include "met.h"            // Base para telas (contorno)
...
#include "C2Grande.h"       // Logo do C2ISC
#include "logoIFGrande.h"   // Logo do IFMG.
TVout TV;
...
const byte ready3 = 37;
const byte ready4 = 33; //MSB
...
// Flag de controle para aguardar resposta do FPGA, monitorando o valor dos pinos de status (ready).
...
/**
 * Método para leitura de dados presentes em uma determinada posição de memória do Processador.
 */
void leMEMInst()
{
  //byte teste[8];  
  ...
  for(int i = 0; i < 4; i++)
  {
    Serial.println(hexaAddressRAM);
    
    converteDecimalBinario(addrInt, binaryAddressRAM, 8);

    // Escreve nos pinos de endereço o vaor correspondente.
    for(int j = 0; j < 8; j++)
    {
      digitalWrite2(pinosAddress[j], binaryAddressRAM[j]);
    }

    // Escreve o valor do opCode "011" nos pinos correspondentes.
    ...
    // Aguarda resposta "ready = 11000" do FPGA, sinalizando que o dado do primeiro byte da memória está pronto nos pinos correspondentes.
    flagReady = false;
    while(!flagReady)
    {
      if ((digitalRead2(ready4) == LOW) && (digitalRead2(ready3) == LOW) && (digitalRead2(ready2) == LOW) && (digitalRead2(ready1) == HIGH) && (digitalRead2(ready0) == LOW))
      {
        flagReady = true;
    ...
    // Lê dados do primeiro byte dos pinos de entrada no Arduino e armazena no vetor de bytes "binByteRAM".
...
void leMEMData()
{
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    while(!flagReady)
    {
      if ((digitalRead2(ready4) == LOW) && (digitalRead2(ready3) == LOW) && (digitalRead2(ready2) == HIGH) && (digitalRead2(ready1) == LOW) && (digitalRead2(ready0) == LOW))
      {
        flagReady = true;
...
void leREGS()
{
    ...
    // Percorre todos os Registradores de propósito geral, ou seja, do endereço 00 ao endereço 31.
    for(int i = 0; i < 34; i++)
    {
      base = 0;
      ...
      for(int j = 0; j < 4; j++)
      {
        // Converte o endereço do registrador a ser lido atualmente (i.e.o valor de da variável "i") para um vetor de bytes binários.
        converteDecimalBinario(i, binaryAddressREG, 8);
  
        // Escreve nos pinos correspondentes, o opCode dessa operação (i.e. "010");
        digitalWrite2(opCode2, LOW);
        digitalWrite2(opCode1, HIGH);
        ...
        // Aguarda resposta "ready = 01001" do FPGA, sinalizando que o dado do primeiro byte do dado do registrador endereçado está pronto nos pinos correspondentes.
        flagReady = false;
        while(!flagReady)
        {
          if ((digitalRead2(ready4) == LOW) && (digitalRead2(ready3) == LOW) && (digitalRead2(ready2) == LOW) && (digitalRead2(ready1) == HIGH) && (digitalRead2(ready0) == HIGH))
          {
            flagReady = true;
          }
        }

        // Lê dados do primeiro byte dos pinos de entrada no Arduino e armazena nas posições corretas do vetor de bytes "binByteREG".
        for(int k = 0; k < 8; k++)
        {
...
void imprimeBaseAbout()
{
  ...
  TV.print(10, 56, "IFMG - 2015 - Formiga/MG");
...
void setup() 
{
  ...
  pinMode2(ready3,INPUT);
  pinMode2(ready4,INPUT);
  ...
  TV.begin(_NTSC, 120, 96); // Resolução da tela.
...
{
  // Verifica se existe algo a ser apagado, ou seja, se a posição do cursor no eixo X é maior que a posição inicial desse.
  if(contAddrRAM > 0)
  {
...
{  
  ...
  // Lê dados do teclado enquanto não ler 8 caracteres ou o joystick for pressionado para cima.
  while( (contAddrRAM < 2) && (sair == false) )
  {
    ...
    // Caso contrário imprime o caractere digitado na posição correta e avança o cursor, 
    // além de incrementar o contador de caracteres digitados.
    else
    {
...
void enviaInstrucao()
{
  ...
  for(int i = 0; i < contLinhasSD; i++)
  {
    ...
    {      
      ...
      // Aguarda resposta "ready = 00001" do FPGA, sinalizando que o dado do primeiro byte do dado do registrador endereçado está pronto nos pinos correspondentes.
      while(! ((digitalRead2(ready4) == LOW) && (digitalRead2(ready3) == LOW) && (digitalRead2(ready2) == LOW) && (digitalRead2(ready1) == LOW) && (digitalRead2(ready0) == HIGH)) ){}
...
void executa()
{
 ...
 flagReady = false;
  while(!flagReady)
  {
    if ((digitalRead2(ready4) == LOW) && (digitalRead2(ready3) == HIGH) && (digitalRead2(ready2) == HIGH) && (digitalRead2(ready1) == HIGH) && (digitalRead2(ready0) == LOW))
    {
      flagReady = true;
    }
  }
